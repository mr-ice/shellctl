...
def test_discover_uses_shell_trace_mock(monkeypatch):
    ...
    assert any(
        any(name in (".bash_profile", ".bashrc") for name in modes[m])
        for m in modes
    )


def test_discover_tcsh_uses_shell_trace_mock(monkeypatch):
    """Force shell-level tracer for tcsh and ensure mock traces expose files."""
    fixtures = os.path.join(os.getcwd(), "tests", "fixtures", "traces")
    monkeypatch.setenv("ENVCONFIG_MOCK_TRACE_DIR", fixtures)
    monkeypatch.setenv("ENVCONFIG_USE_SHELL_TRACE", "1")
    ...
    assert any(
        any(name in (".cshrc", ".login", ".tcshrc") for name in modes[m])
        for m in modes
    )
