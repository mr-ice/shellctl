...
From the env-config repo (clones into `tcsh-src/tcsh-git/`, patches, builds, copies binary to `tcsh-src/tcsh`):
...
patch -p1 < /path/to/env-config/patches/tcsh-closem-preserve-xtracefd.patch
patch -p1 < /path/to/env-config/patches/tcsh-xtrace-filepath.patch
patch -p1 < /path/to/env-config/patches/tcsh-allow-l-with-args.patch
patch -p1 < /path/to/env-config/patches/tcsh-sourcetrace.patch
./configure && make
```
...
or `BASH_XTRACEFD`), shaped like env-config’s parser expects:
...
From the env-config repo (downloads `bash-$(BASH_VERSION).tar.gz`, unpacks under `bash-src/`, patches, builds, copies the binary to `bash-src/bash`):
...
patch -p1 < /path/to/env-config/patches/bash-sourcetrace.patch
./configure && make
```

The `bash` binary is produced in the build tree (often `./bash`). Point env-config at it via:

- `ENVCONFIG_BASH_PATH=/path/to/bash-5.2/bash`
- Or install/copy the binary to `bash-src/bash` at the env-config project root
...
```bash
cd bash-5.2
patch -p1 < /path/to/env-config/patches/bash-xtrace-fileline.patch
./configure && make
```
